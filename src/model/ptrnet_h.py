...
class PtrNetH:
    ...
    def forward(self):
        ...
        logits = self.training_decoding_layer(
            enc_output, self.src, self.src_len, knowledge_vec, enc_state, tgt_em[:, :-1], self.tgt_len - 1
        )

        # decoding in testing
        if not self.beam_search:
            predicted_ids = self.inference_decoding_layer(
                enc_output, self.src, self.src_len, knowledge_vec, enc_state, beam_search=self.beam_search
            )
        else:
            # tiled to beam size
    ...
    def training_decoding_layer(self, memory, memory_ids, memory_len, knowledge_vec, cell_state, tgt_em, tgt_len):
        with tf.variable_scope('decoder', reuse=False):
            # add pointer mechanism to decoder cell
