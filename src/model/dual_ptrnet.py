...
from .module.dual_pointer_wrapper import DualPointerWrapper
...
class DualPtrNet:
    ...
    def forward(self):
        ...
        # encoding
        triple_enc_output, knowledge_vec = self.knowledge_encoding_layer(topic_em, self.topic_len, triple_em, self.triple_len)
        ...
        logits = self.training_decoding_layer(
            enc_output, self.src, self.src_len,
            triple_enc_output, self.triple, self.triple_len,
            knowledge_vec, enc_state, tgt_em[:, :-1], self.tgt_len - 1
        )
        ...
        if not self.beam_search:
            ...
            enc_output, self.src, self.src_len,
            triple_enc_output, self.triple, self.triple_len,
            knowledge_vec, enc_state, beam_search=self.beam_search
            )
        else:
            # tiled to beam size
            tiled_enc_output = tf.contrib.seq2seq.tile_batch(enc_output, multiplier=self.beam_size)
            tiled_src = tf.contrib.seq2seq.tile_batch(self.src, multiplier=self.beam_size)
            tiled_src_len = tf.contrib.seq2seq.tile_batch(self.src_len, multiplier=self.beam_size)
            tiled_triple_enc_output = tf.contrib.seq2seq.tile_batch(triple_enc_output, multiplier=self.beam_size)
            ...
            predicted_ids = self.inference_decoding_layer(
                tiled_enc_output, tiled_src, tiled_src_len,
                tiled_triple_enc_output, tiled_triple, tiled_triple_len,
    ...
    def knowledge_encoding_layer(self, topic_em, topic_len, triple_em, triple_len):
        ...
        return triple_enc_output, knowledge_vec
    ...
    def training_decoding_layer(self,
                                memory_a, memory_a_ids, memory_a_len,
                                memory_b, memory_b_ids, memory_b_len,
                                knowledge_vec, cell_state, tgt_em, tgt_len):
        with tf.variable_scope('decoder', reuse=False):
            # add dual pointer mechanism to decoder cell
            decoder_cell = DualPointerWrapper(
                self.decoder_cell,
                self.attention_size,
                memory_a,
                memory_a_ids,
            ...
            dec_initial_state = decoder_cell.zero_state(batch_size=tf.shape(memory_a)[0], dtype=tf.float32)
    ...
    def inference_decoding_layer(self,
                                 memory_a, memory_a_ids, memory_a_len,
                                 memory_b, memory_b_ids, memory_b_len,
                                 knowledge_vec, cell_state, beam_search):
